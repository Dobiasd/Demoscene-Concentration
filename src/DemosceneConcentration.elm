...
import List exposing (map, concat, length, map2, (::), foldr, filter, all, isEmpty)
import Signal exposing (merge, Signal, sampleOn, dropRepeats, foldp)
...
import Common.Display exposing (winPosToGamePos, displayFullScreen, FPSCounter, makeFPSCounter, stepFPSCounter)
import Card
...
input : Signal Input
input =
    Signal.map Input actions


steps : Signal Action
steps =
    Signal.map Step speed
...
generateCards seed =
    ...
    in
        map2
            (\effect box -> Card.make effect box)
            (effects ++ effects)
            shuffledBoxes
...
stepPlayFlipCards tapPos cards =
    ...
    in
        ...
        flipCardsHit tapPos
            <| if faceUpCount == 2 then
                putFaceDown cardsNotDone
               else
                cardsNotDone
    ...
    let
        ...
        cards' =
            getDoneCards cards
                ++ cardsNotDoneDown'
                ++ if foundPair then
                    putCardsFaceDown cardsNotDoneUp'
                   else
                    cardsNotDoneUp'
    in
        { game
...
stepTap gameTapPos ({ state, cards } as game) =
    case state of
        Start ->
            stepTapStart gameTapPos game

        Play ->
            stepTapPlay gameTapPos game
...
stepGame { action } ({ state, time } as game) =
    case action of
        Step delta ->
            stepDelta delta game

        Tap tapPos winDims ->
            stepTap (winPosToGamePos tapPos winDims) game
...
displayFPS { lastVal } =
    ...
    in
        txt (Text.height fpsTextHeight) ("FPS: " ++ (toString <| lastVal))
            |> toForm
            |> move ( fpsTextPosX, fpsTextPosY )


{-| Show the final Effect
-}
displayWon : Game -> Form
displayWon ({ wonEffect } as game) =
    Eff.display wonEffect
...
displayNotYetDone ({ time, cards } as game) =
    let
        ...
        timeTextForm =
            txt (Text.height timeTextHeight) (toString <| roundTime time)
                |> toForm
                |> move ( 0, timeTextPosY )
    in
        group
...
display ({ state, fpsCounter } as game) =
    group
        [ case state of
            ...
            _ ->
                displayNotYetDone game
        , displayFPS fpsCounter
        ]


main =
    Signal.map2 (displayFullScreen display) gameState Window.dimensions
