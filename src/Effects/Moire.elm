...
import Graphics.Collage exposing (solid, path, traced, Form, group, rect, filled)
import List exposing (map, length, map2)
import Effects.Effect as Eff
import Common.Vector exposing (Vector, vector, Transform3D, applyTransform3D, rotateZ)
import Effects.Starfield as Starfield


type alias State =
    { time : Float, background : Eff.Effect }
...
make : Eff.Effect
make =
    moire { time = 0, background = Starfield.make Starfield.Colored 0.2 64 }
