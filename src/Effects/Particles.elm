...
import Graphics.Collage exposing (oval, gradient, circle, rect, move, Form, filled, group)
...
import Common.Display exposing (PositionedForm, positionedForm, displayPositionedForms, isPosOK, decomposeColor)
import Common.Vector exposing (Vector, vector, Transform3D, applyTransform3D, rotateY, project2d, addVec, move3, distTo, multVec, subVec)


type alias Ball =
    Positioned (Moving (Colored {}))


type alias State =
    { time : Float, balls : List Ball }


ball : Float -> Float -> Float -> Float -> Float -> Float -> Color -> Ball
ball x y z vx vy vz col =
    { x = x, y = y, z = z, vx = vx, vy = vy, vz = vz, col = col }


make : Eff.Effect
make =
    particles { time = 0, balls = generateBalls 64 1.23 }
...
displayBallShadow angle ({ x, y, z } as b) =
    ...
    in
        positionedForm
            (oval radius (0.3 * radius) |> gradient grad)
...
pullToRoot delta ({ x, y, z, vx, vy, vz } as b) =
    let
        ...
        pull = diff `multVec` (1.0e-5 * delta)
    in
        { b
            | vx = vx + pull.x
            , vy = vy + pull.y
...
displayStar star =
    ...
    in
        positionedForm
            (circle 0.6 |> filled white |> move ( x, y ))
            { x = x, y = y, z = star.z }
...
display ({ time, balls } as state) =
    let
        ...
        starForms =
            map (applyTransform3D m) staticStars
                |> map displayStar
                |> filter isPosOK
                |> map .f
        ...
        ballForms =
            map (displayBallWithShadow rotateAngle) movedBalls
                |> concat
