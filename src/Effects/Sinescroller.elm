...
type alias State =
    { message : String, time : Float, cube : Eff.Effect }
...
make message =
    sinescroller
        { message = message
        , time = 0
        , cube = Cube.make [] (rgba 255 255 255 7.0e-2)
        }
...
deconcat =
    String.foldr (\c acc -> String.fromList [ c ] :: acc) []
...
charCol time { x, y } =
    hsl (1.0e-3 * time + 3.0e-2 * x) 1 0.5


type alias ScrollerChar =
    Colored (Positioned { s : String })


scrollerChar : String -> Point -> Color -> ScrollerChar
scrollerChar s pos col =
    { s = s, x = pos.x, y = pos.y, z = 0, col = col }
