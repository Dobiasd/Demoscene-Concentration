...
import Common.Display exposing (displayPositionedForms, PositionedForm, isPosOK, positionedForm)
import Common.Types exposing (WithRadius, WithNormal, Colored, Positioned, Point, point2D)


type alias State =
    { time : Float, background : Eff.Effect, discs : List Disc }
...
make =
    tunnel
        { time = 0
        , background = Starfield.make Starfield.BW 5.0e-2 64
        , discs = []
        }
...
type alias Disc =
    WithRadius (Colored (Positioned (WithNormal {})))


disc : Float -> Float -> Float -> Float -> Float -> Float -> Color -> Disc
disc x y z nx ny nz c =
    { x = x, y = y, z = z, col = c, r = 400, nx = nx, ny = ny, nz = nz }
...
generateNewRingDisc time num =
    let
        calcCol v = hsla (v * 6.0e-2) 1 0.5 1
        ...
        xOff = off * (cos (1.04e-3 * time) + cos (5.6e-4 * time))

        yOff = off * (cos (4.5e-4 * time) + cos (8.7e-4 * time))
    in
...
stepDisc delta ({ z } as disc) =
    { disc | z = z + 5.0e-2 * delta }
...
step ({ time, discs, background } as state) delta =
    let
        ...
        newDiscs =
            if (isEmpty oldDiscs) || (last oldDiscs).z > -105 then
                generateNewDiscRing time
            else
                []
