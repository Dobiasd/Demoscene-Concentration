...
import Graphics.Collage exposing (solid, LineCap(Round), path, traced, Form, group, rect, filled, move, scale)
...
import Common.Vector exposing (Vector, vector, angle2D, vector2DFromAngle, multVec, addVec, subVec)
import Effects.Effect as Eff
import Debug


type alias State =
    { time : Float, points : List Vector, stepCount : Int }
