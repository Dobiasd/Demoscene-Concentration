...
import Graphics.Collage exposing (group, move, scale, Form, solid, LineJoin(Smooth), LineCap(Round), rect, outlined, filled)
...
type alias Card =
    Boxed { effect : Eff.Effect, status : Status, doneTime : Time }
    ...
    let
        texture =
            case card.status of
                ...
                FaceUp ->
                    group [ Eff.display card.effect, border ]

                Done ->
                    group
                        [ Eff.display card.effect
                        , border
                        , doneOverlay
                        ]
    in
...
doneOverlay : Form
doneOverlay =
    rect 200 200 |> filled (rgba 0 0 0 0.4)
...
allEqual cards =
    let
        ...
        equalName (Eff.Effect e1) (Eff.Effect e2) =
            e1.name == e2.name
    in
        if length es < 2 then
            True
        else
            all (equalName (unsafeHead es)) (unsafeTail es)
